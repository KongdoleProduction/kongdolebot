    ...
    if (payload.type === "url_verification") {
      ...
      if (payload.event.type === "app_mention") {
        ...
        if (!user_id) { 
          ...
          if (cmd_name === '전체') {
          ...
          } else if (cmd_name == 'dm') { /* direct message */
            let re_dm = /\s*<\@([\S]+)>\s([\s\S]+)/; /* [@username] [msg] */
            let arr_dm = re_dm.exec(cmd_contents);
            let target_user_id = arr_dm[1];
            let target_msg = arr_dm[2];
            ...
            let target_channel = dm_channels.find(function(element) {
              return element.user === target_user_id;
            });
